...
async function createBranch() {
    try {
        ...
        for (const b of remoteBranches) {
            ...
            if (localBranches.includes(b.replace(/^origin\//, '')))
                continue;
            baseItems.push({
                label: `$(remote) ${b}`,
                description: 'remote',
        ...
        const newName = await vscode.window.showInputBox({
            ...
            validateInput: (value) => {
                ...
                if (value.length > 255)
                    return 'Branch name is too long (max 255 chars)';
                return null;
            },
        });
        ...
        terminal_1.gitm8Terminal.sendCommand(branchCmd);
        // ── Step 5: Offer to publish ──
        const publishAction = '$(cloud-upload) Push branch';
        const result = await vscode.window.showInformationMessage(`Branch "${newName.trim()}" created from "${baseRef}". Push it now?`, publishAction);
        if (result === publishAction) {
            // Small delay to let the checkout finish
            setTimeout(() => {
                terminal_1.gitm8Terminal.sendCommand(`git push -u origin "${newName.trim()}"`);
            }, 500);
        }
    }
    catch (err) {
        vscode.window.showErrorMessage(`gitm8: Branch creation failed — ${err instanceof Error ? err.message : String(err)}`);
    }
}
// ── Git helpers ──
async function getCurrentBranch() {
    const result = await (0, git_1.execGit)(['rev-parse', '--abbrev-ref', 'HEAD']);
    return result.trim() || 'main';
}
async function getLocalBranches() {
    const result = await (0, git_1.execGit)(['branch', '--format=%(refname:short)']);
    return result
        .split('\n')
        .map((b) => b.trim())
...
async function getRemoteBranches() {
    try {
        const result = await (0, git_1.execGit)(['branch', '-r', '--format=%(refname:short)']);
        return result
            .split('\n')
            .map((b) => b.trim())
