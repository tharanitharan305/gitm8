...
import { getApiKey, storeApiKey, deleteApiKey } from '../secrets';
...
export function openConfigPanel(): void {
  ...
  panel.webview.onDidReceiveMessage(async (message) => {
    switch (message.type) {
      ...
      case 'saveApiKey':
        await storeApiKey(message.payload.apiKey);
...
async function sendConfigToWebview(panel: vscode.WebviewPanel): Promise<void> {
  ...
  panel.webview.postMessage({
    ...
    payload: {
      ...
      tone: config.get<string>('tone'),
      customTone: config.get<string>('customTone'),
      commitStyle: config.get<string>('commitStyle'),
      maxDiffChars: config.get<number>('maxDiffChars'),
      pipelinePrecheck: config.get<boolean>('pipelinePrecheck'),
      pipelineAutoPush: config.get<boolean>('pipelineAutoPush'),
      apiKeyExists: !!apiKey,
...
async function saveConfig(payload: any): Promise<void> {
  const config = vscode.workspace.getConfiguration('gitm8');
